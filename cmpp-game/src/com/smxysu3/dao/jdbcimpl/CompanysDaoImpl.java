...
public class CompanysDaoImpl implements CompanysDao {
	...
	public Companys selectByCompanytag(String companytag,String cpn) {
		...
		String dbIp = "xiangtone_dbip";
			...
			int sendFlag = 0;
		String sql = "select id,company,cpip,game,addate,flag from companys  where companytag = '" + companytag + "' and provid = '"+ provId +"'";
		...
		try {
			...
			if(resultSet.next()){
				...
				if(provSendSwitch == 41 || provSendSwitch == 42 || provSendSwitch == 43 || provSendSwitch == 44){
					/*
					company.setCompany(resultSet.getString("company") + "pf");
					sendFlag=1;
					*/
				...
				company.setFlag(sendFlag);
				  ...
				  //sql = "select id,company,cpip,game,addate,flag from companys  where companytag = '" + companytag.substring(0,6) + "'";
				 
				String tempCompanyTag = "";
				if(companytag.length() >= 6){
					sql = "select id,company,cpip,game,addate,flag from companys  where companytag = '" + companytag.substring(0,6) + "' and provid = '"+ provId +"'";
					tempCompanyTag = companytag.substring(0,6);
				}
				else{
					sql = "select id,company,cpip,game,addate,flag from companys  where companytag = '" + companytag.substring(0,5) + "' and provid = '"+ provId +"'";	
				...
				//System.out.println(resultSet.getInt("id"));
					...
					//////////////
					sql = "select id,company,cpip,game,addate,flag from companys  where companytag = '" + companytag.substring(0,4)+ "' and provid = '"+ provId +"'";
					//preParedStatement = connection.prepareStatement(sql);
					//preParedStatement.setString(1, companytag.substring(0,4));
					resultSet =  mydb.executeQuery(sql);
					company = new Companys();
					if(resultSet.next()){
			...
			if(company.getCompany()==null){
					///////////////////
					//System.out.println("the companytag is:--------------==============================" + companytag);
					//sql = "select id,company,cpip,game,addate,flag from companys  where companytag = '" + companytag + "' and provid = '9999'";
					System.out.println("the null sql is:++++++++++++++++++++++++++++++++++++++++++++++++++++++++" + sql);
					resultSet =  mydb.executeQuery(sql);
				...
				if(resultSet.next()){
					...
					String tempCompanyTag = "";
					if(companytag.length() >= 6){
						sql = "select id,company,cpip,game,addate,flag from companys  where companytag = '" + companytag.substring(0,6) + "' and provid = '9999'";
						tempCompanyTag = companytag.substring(0,6);
					}
					else{
						sql = "select id,company,cpip,game,addate,flag from companys  where companytag = '" + companytag.substring(0,5) + "' and provid = '9999'";	
						tempCompanyTag = companytag.substring(0,5);
					}
					resultSet =  mydb.executeQuery(sql);
					//preParedStatement = connection.prepareStatement(sql);
					...
					//System.out.println(resultSet.getInt("id"));
						...
						//////////////
						sql = "select id,company,cpip,game,addate,flag from companys  where companytag = '" + companytag.substring(0,4)+ "' and provid = '9999'";
						//preParedStatement = connection.prepareStatement(sql);
						//preParedStatement.setString(1, companytag.substring(0,4));
						resultSet =  mydb.executeQuery(sql);
						company = new Companys();
