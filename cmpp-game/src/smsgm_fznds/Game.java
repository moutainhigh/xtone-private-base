...
public class Game {
	...
	public int execMain(String cpn,String itemaction,String ismgid,String linkid,Integer cpnType,String msgId,Vector cpnSet,Vector cpnGet,Vector msgSet,Vector costSet,Vector ismgSet,Vector linkidSet,Vector cpntypeSet,Vector msgIdSet){
		...
		/////////////////////
			//String userInput = itemaction;
			String tableName = "tempcompany_mo";//GameMtHandle.FormatSysTime.getCurrentTime("yyyyMMdd") + "company_mo";
			if(linkid == null){
				linkid = "";	
			}
			...
			if(!retMsg.equals("")){
				msgId = System.nanoTime() + linkid.substring(linkid.length() - 7,linkid.length());
				insertMt(cpn,userInput,retMsg,linkid,msgId);
	...
	private void insertMo(String cpn,String itemaction,String linkid,String msgId,Companys company,String tableName){
	...
	//用于下行入库
	private void insertMt(String cpn,String userInput,String retMsg,String linkid,String msgId){
			...
			try{
					...
					smsMtHandle.compMt.setMsgId(msgId);
	...
	private void smsProcess(String cpn,String itemaction,String linkid,String msgId,String tableName){
		...
		int moLimit = Integer.parseInt((String)ConfigManager.getInstance().getConfigItem("molimit" ," molimit not found!"));//10;//上行的上限
