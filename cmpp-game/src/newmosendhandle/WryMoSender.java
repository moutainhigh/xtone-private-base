...
public class WryMoSender extends MoSender{
	String company = "";
	String compUrl = "";
	String sendStat = "";
	...
	public WryMoSender(String company){
	...
	public void getCompanyUrl(){
		...
		System.out.println("wry url is:" + this.compUrl);
	}
	//public void sendMo(){
	public void run(){
		while(true){
				try{
					...
					System.out.println("wry exception");
					e.printStackTrace();
				}
				System.out.println("" + this.sendSecond);
				///////////////////////
				if(0 < this.mosList.size() && this.mosList.size()< 8 && this.sendSecond < 4){
				...
				moCounter.mydb.dbclose();
				try{
									System.out.println("Sjf start to rest......");
	...
	private void writeLog(String logStr){
		...
		PropertyConfigurator.configure("wrylog4j.properties"); 
		//logStr = this.compUrl + "\n" + logStr;
