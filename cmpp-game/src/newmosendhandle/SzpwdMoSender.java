...
public class SzpwdMoSender extends MoSender{
	String company = "";
	String compUrl = "";
	String sendStat = "";
	...
	public SzpwdMoSender(String company){
	...
	public void getCompanyUrl(){
		...
		System.out.println("szpwd url is:" + this.compUrl);
	}
	//public void sendMo(){
	public void run(){
		while(true){
				try{
					...
					//e.printStackTrace();
					try{
						System.out.println("szpwd exception");
						e.printStackTrace();
						moCounter.mydb.dbclose();	
					}catch(Exception e1){
							e1.printStackTrace();
				...
				moCounter.mydb.dbclose();
				try{
					System.out.println("Szpwd start to rest......");
	...
	private void writeLog(String logStr){
		...
		PropertyConfigurator.configure("szpwdlog4j.properties"); 
		//logStr = this.compUrl + "\n" + logStr;
