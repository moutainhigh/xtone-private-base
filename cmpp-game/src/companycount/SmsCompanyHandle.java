...
public class SmsCompanyHandle {
	...
	public void smsCompanyMoHandle(){//处理上行
		if(this.compMo.getGame().equals("1000")){
			insertMo();
		}
		else if(!checkLinkid(this.compMo.getLinkId())){
			insertMo();
		}
	...
	public boolean checkLinkid(String linkId){
		boolean checkFlag = true;
		String tableName =  FormatSysTime.getCurrentTime("yyyyMMdd") + "company_mo";
		String strSql = "select id from " + tableName + " where linkid='" + linkId + "'";
		try{
	...
	private void insertMo(){
		try{
			...
			//String tableName =  FormatSysTime.getCurrentTime("yyyyMMdd") + "company_mo";//changed at 090806
	...
	private void insertMt(){
		try{
			...
			String strSql = "insert into company_mt set company='" + companyName + "',game='" + this.compMt.getGame() + "',content='" + this.compMt.getContent() + "',msgid='" + this.compMt.getMsgId() + "',cpn = '" + this.compMt.getCpn() + "',addate='" + FormatSysTime.getCurrentTimeA() + "',sendstate='-3'";
			mydb.executeUpdate(strSql);
		}catch(Exception e){
			e.printStackTrace();
		}
