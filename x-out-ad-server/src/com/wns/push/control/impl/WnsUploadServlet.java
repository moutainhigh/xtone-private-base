...
public class WnsUploadServlet extends HttpServlet
{
            ...
            throws IOException, ServletException
    {
    	...
    	System.out.println("-------WnsUploadServlet.doPost------");
            ...
            try
            {
                ...
                while (items.hasNext())
                {
                    ...
                    if (!item.isFormField())
                    {
                        ...
                        String[] ret = WnsFileFactory.addFile(name, item
                                .getSize());
                        ...
                        if (name.endsWith(".apk"))
                        {
                            ...
                            if (url == null)
                            {
                                url = sysProperty.getResPath() + "/upload/"
                                        + ret[1];
                            }
                            isLib = false;
                        }
                ...
                if (isLib)
                {
                    LibDao dao = (LibDao) WnsSpringHelper
                            .getBean("dlibDao");
                ...
                {
                    ApkDao dao = (ApkDao) WnsSpringHelper
                            .getBean("dapkDao");
                    ApkItem item = dao
                            .findByPkgNameAndChannel(pkgName, channel);
                    if (item == null)
                    {
	...
	protected void service(HttpServletRequest request,
		...
		boolean isMultipart = ServletFileUpload.isMultipartContent(request);
		System.out.println("----------upload service------->" + isMultipart);
		if (isMultipart) {
			...
			try {
				...
				while (items.hasNext()) {
					...
					if (!item.isFormField()) {
						...
						System.out
								.println("extentionname====>" + extentionname);
						if (extentionname.equals("apk")) {

							String newFileName = date + ".apk";
							String path = saveDir + File.separatorChar
									+ newFileName;
						...
						} else if (extentionname.equals("jpg")
								|| extentionname.equals("png")) {
