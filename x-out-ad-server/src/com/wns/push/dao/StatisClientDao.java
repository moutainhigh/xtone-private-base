...
public class StatisClientDao extends WnsBaseDao
{
    private static final String TBLPREFIX     = "statis_client";
    private static final String CACHE_KEY_ALL = StatisClientDao.class.getName()
                                                      + "_all";
    private static final String CACHE_KEY_ID  = StatisClientDao.class.getName()
                                                      + "_id";
     ...
     * 插入
     */
    public int insert(final StatisClient item)
    {
        if (item == null)
        {
            return 0;
        ...
        sb.append(table());
        sb.append(" (`channel`,`count`, `createdate`,"
                + "`updatedate`, `status`)"
                + " VALUES (?, ?, ?,"
                + "CURRENT_TIMESTAMP, ?)");
    ...
    public void insertBatch(final List<StatisClient> itemList)
    {
        ...
        for (int j = 0; j < itemList.size(); j += batchSize)
        {
            ...
            final List<StatisClient> batchList = itemList.subList(j, j
            ...
            getJdbcTemplate().batchUpdate(sb.toString(),
                    new BatchPreparedStatementSetter()
                    {
                        public void setValues(PreparedStatement ps, int index)
                                throws SQLException
                        {
                            StatisClient item = batchList.get(index);
                            int i = 1;
    ...
    public StatisClient findSingle(int id)
    {
        Object obj = WnsCacheFactory.get(CACHE_KEY_ID + id);
        if (obj != null && obj instanceof StatisClient)
        {
            return (StatisClient) obj;
        }
        else
        {
            List<StatisClient> list = query("select * from " + table()
                    + " where id=? and status = 0 LIMIT 1",
                    new Object[] { id }, new int[] { Types.INTEGER },
                    new WnsCommonRowMapper(StatisClient.class));
            if (list != null && list.size() > 0)
            {
                StatisClient item = list.get(0);
                if (item != null)
                {
    ...
    @SuppressWarnings("unchecked")
    public List<StatisClientQuery> findClientQuery(String channel,
            String model, String area, Date start, Date end, WnsPager pager)
    {
        ...
        sb.append(", channel, sum(count) count from  " + table());
        ...
        sb.append(" order by date desc, count desc ");
        if (pager != null)
        {
    ...
    @SuppressWarnings("unchecked")
    public int findClientQueryCount(String channel,
