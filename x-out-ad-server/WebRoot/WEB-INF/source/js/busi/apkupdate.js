		...
		$('#dataList').empty();
			if(data&&data.rows){
				$.each(data.rows,function(i,row){
					$('#dataList').append('<tr><td>'+row.name+'</td><td>'+row.pkgname+'</td><td>'+row.version+'</td><td>'+row.versioncode+'</td><td>'+row.url+'</td><td>'+row.channel+'</td><td>'+row.phonetype+'</td><td><a href="#" onclick="delete(\''+row.id+'\')">删除</a></td></tr>');
				});
			}else{
				$('#dataList').append('<tr><td colspan="9">暂无数据</td></tr>');
			}
			
			$.pager({id:"result",totalrows:data.totalrows,pagesize:data.pagesize,currentpage:data.currentpage},loadApk);
	});
};
		...
		if (data) {
			if (data.channels) {//渠道列表
				...
				$.each(data.channels, function(i, channel) {
					cpOptionHtml += "<option value='" + channel.channelid
