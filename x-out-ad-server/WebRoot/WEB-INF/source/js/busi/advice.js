...
var uploadApp = function() {
	...
	$.ajaxFileUpload({
		url : "http://localhost:8080/hlpads/upload",   //submit to UploadFileServlet
		secureuri : false,
		...
		*/
		...
		$('#calendar').fullCalendar({
			...
			events:  function(start, end , callback){//生成日历
				loadEvents(start, end, "4", callback);                
			},
			//单日点击
			...
			eventClick: function(event, jsEvent, view) {
				...
				totalRows : pager.totalrows
					...
					if (data && data.event) {
						...
						$('#subtype').val(data.event.subtype);
