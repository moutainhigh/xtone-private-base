...
var detailtime={"00":"00:00~01:00","01":"01:00~02:00","02":"02:00~03:00","03":"03:00~04:00","04":"04:00~05:00","05":"05:00~06:00","06":"06:00~07:00","07":"07:00~08:00","08":"08:00~09:00","09":"09:00~10:00","10":"10:00~11:00","11":"11:00~12:00","12":"12:00~13:00",
	"13":"13:00~14:00","14":"14:00~15:00","15":"15:00~16:00","16":"16:00~17:00","17":"17:00~18:00","18":"18:00~19:00","19":"19:00~20:00","20":"20:00~21:00","21":"21:00~22:00","22":"22:00~23:00","23":"23:00~24:00"};
/*
 * 加载支付方式
 * 
 */
var loadPayType = function (id,callback){
	...
	$.reqPostData( {
		...
		if (data && data.paytype) {
			...
			$.each(data.paytype, function(i, paytype) {
				var selectedStyle = "";
				if (id != null && $.trim(id) == paytype.id)
					selectedStyle = "selected='selected'";
				$('#paytype').append("<option value='" + paytype.id + "'" + selectedStyle +">" + paytype.name
						+ "</option>");
			});
 ...
 * 
 * **/
var loadUserCpOrSrc = function (callback){
	
	$.reqPostData( {
		reqType : 'F9902007'
	},function(data){
		if (data) {
			if (data.srcList){//拼成渠道-cp联动
				var srcOptionHtml = '<select id="src" class="goods_select" onchange="showCpList();"><option value="-1" class="goods_select">全部</option>';
				$.each(data.srcList, function(i, src) {
					srcOptionHtml+="<option value='" + src.id + "'>" + src.name +  "</option>";
				});
				srcOptionHtml += '</select>';
				srcOptionHtml += '<select id="cp" class="good_select" onchange="setCpValue();"><option value="-1" class="goods_select">全部</option></select>';
				$('.customquery table tr:first').after('<tr><td align="right">选择渠道CP:</td><td>' + srcOptionHtml + "</td><tr>");
				
			}else if (data.cpList){//拼成cp列表
				var cpOptionHtml = '<select id="cp" class="goods_select" onchange="setCpValue();"><option value="-1" class="goods_select">全部</option>';
				$.each(data.cpList, function(i, cp) {
					var text = cp.name;
					if (cp.level)
						text = cp.name + '';
					cpOptionHtml+="<option value='" + cp.id + "'>" + text +  "</option>";
				});
				$('.customquery table tr:first').after('<tr><td align="right">选择渠道CP:</td><td>' + cpOptionHtml + "</td><tr>");
			}else{//获取当前cp
				//初始化当前cp统计类型,商品,计费点类型
				if (data.cp){
					$.each(data.cp, function(i, p) {
						loadCpCondtion(p.id);
					});
				}
...
var setCpValue = function(){
	...
	if (value != '-1'){
		...
		$('#cpid').val('');
		$('#channel').empty();
		$('#channel').append('<option value="-1" class="goods_select">全部</option>');
		$('#paytype').empty();
		$('#paytype').append('<option value="-1" class="goods_select">全部</option>');
		$('#point').empty();
		$('#point').append('<option value="-1" class="goods_select">全部</option>');
		$('#product').empty();
		$('#product').append('<option value="-1" class="goods_select">全部</option>');
	}
		...
		SERVERURL + '109',
		{
			...
			if(data&&data.rows){
				$.each(data.rows,function(i,row){
					$('#dataList').append('<tr><td>'+row.date+'</td><td>'+paramproduct[defaultstr(row.cppid)]+'</td><td>'+parampaytype[defaultstr(row.paytype)]+'</td><td>'+row.usercount+'</td><td>'+row.paycount+'</td><td>'+defaultfloat(row.money)+'</td><td><a href="#" onclick="initdetail(\''+row.date+'\','+row.cpcid+','+row.cppid+','+row.cppaypoint+','+row.paytype+')">详细</a></td></tr>');
				...
				$('#dataList').append('<tr><td colspan="9">暂无数据</td></tr>');
			}
			
			$.pager({id:"result",totalrows:data.totalrows,pagesize:data.pagesize,currentpage:data.currentpage},doDelete);
		});
	//}else{
	//	alert("请至少选择一个cp分析!");
	//}
...
var loaddetail = function (data) {
	...
	if(data&&data.rows){
		...
		$.each(data.rows,function(i,row){
			...
			$('#showdetaildata').append('<tr><td>'+detailtime[row.date]+'</td><td>'+defaultint(row.usercount)+'</td><td>'+(defaultint(row.usercount)>0?(defaultfloat(row.money)/defaultint(row.usercount)).toFixed(2):0)+'</td><td>'+defaultint(row.paycount)+'</td><td>'+(defaultint(row.paycount)>0?(defaultfloat(row.money)/defaultint(row.paycount)).toFixed(2):0)+'</td><td>'+defaultfloat(row.money)+'</td>');
		});
		$('#sumdetaildata').append('<tr><td>24</td><td>'+defaultint(sumusercount)+'</td><td>'+(defaultint(sumusercount)>0?(defaultfloat(sumuserpay)/defaultint(sumusercount)).toFixed(2):0)+'</td><td>'+defaultint(sumcount)+'</td><td>'+(defaultint(sumcount)>0?(defaultfloat(sumuserpay)/defaultint(sumcount)).toFixed(2):0)+'</td><td>'+defaultfloat(sumuserpay).toFixed(2)+'</td>');
		var  series = [{
                     name: '交易金额',
                     data: arr
                  }];
...
$(document).ready(function(){
	...
	options= {
            ...
            tooltip: {
                formatter: function() {return '<b>'+ this.series.name +'</b><br/>'+ this.x +': '+ this.y +'元';}
            }
     };
	$('#product').change(function(){changeProduct($(this).children('option:selected').val());});
});
