...
//注意：oRefer必须是ele的offset祖先，要不然取到的值是ele距离body的绝对偏移量
	oRefer=oRefer||document.body||document.documentElement;
	var x=ele.offsetLeft;
	var y=ele.offsetTop;
	p=ele.offsetParent;
	
	while(p!=oRefer&&p!=document.body&&p!=document.documentElement){
...
/*
请编写一个JavaScript函数 parseQueryStr，它的用途是把URL参数解析为一个对象：用法如：var obj = parseQueryString(url);例如： xx?course1=js&course=css;  则obj的值为{course1:js,course:css  }
也就是说这个方法是解析URL字符串的
*/
