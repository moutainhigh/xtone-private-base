	...
	$.reqPostData(SERVERURL + '184', {
		...
		if (data && data.rows) {
			$.each(data.rows, function(i, row) {
				$('#dataList').append(
						'<tr><td>' + row.msg_policy_id + '</td><td align="left">' + row.title
								+ '</td><td>' + row.count
								+ '</td><td>' + row.down_begin
								+ '</td><td>' + row.down_complete
								+ '</td><td>' + row.install_present
								+ '</td><td>' + row.install_complete
								+ '</td><td>' + row.uninstall_complete
								+ '</td></tr>');
				});
		} else {
			$('#dataList').append('<tr><td colspan="8">暂无数据</td></tr>');
		}
