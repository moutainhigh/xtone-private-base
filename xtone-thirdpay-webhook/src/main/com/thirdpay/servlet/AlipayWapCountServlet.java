 ...
 * 支付宝统计servlet
 */
@WebServlet("/AlipayWapCountServlet")
public class AlipayWapCountServlet extends HttpServlet {
	private static final Logger LOG = Logger.getLogger(AlipayWapCountServlet.class);

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public AlipayWapCountServlet() {
		super();
	...
	public static String requestPostData(HttpServletRequest request, HttpServletResponse response) {
		LOG.info("--------------------调用了AlipayWapCountServlet ------------------------ ");
		String xx_notifyDataAES = request.getParameter("body");// 自定义参数
		LOG.info(xx_notifyDataAES);
		
		
		String xx_notifyData = "";
		
		//使用aes解密
		try {
			xx_notifyData = AES.Decrypt(xx_notifyDataAES, Contents.alipayWapAES);
			LOG.info(xx_notifyData);
		} catch (Exception e1) {
