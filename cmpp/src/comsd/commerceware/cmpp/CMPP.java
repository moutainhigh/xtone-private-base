...
//            cmppe_pack, cmppe_head, conn_desc, OutOfBoundsException, 
//            cmppe_login, MD5, UnknownPackException, DeliverFailException, 
//            cmppe_result, cmppe_submit_result, cmppe_login_result, cmppe_us_user, 
//            cmppe_deliver_result, cmppe_cancel, cmppe_submit
...
public final class CMPP
{
	...
	//连接网关
	public void cmpp_connect_to_ismg(String host, int port, conn_desc conn)
        throws IOException
    {
        Socket s = null;
        try
  ...
  //login ismg
   public void cmpp_login(conn_desc conn, cmppe_login cl)
        throws IOException, OutOfBoundsException
    {
        ...
        try
        {
            ...
            byte[] bufMd5 = md5.getMD5ofStr(md5Byte, lengthMd5);// get md5 for icp_auth
            body_len +=tools.strcpy(buf,bufMd5,body_len+ch.pk_len); // add icp_auth
            
            body_len +=tools.strcpy(buf,cl.icp_version,body_len+ch.pk_len); //add icp_version
            body_len +=tools.strcpy(buf,cl.icp_timestamp,body_len+ch.pk_len); // add icp_timestamp
             // body_len +=tools.strcpy(buf,(short)cl.icp_timestamp,body_len+ch.pk_len); // add icp_timestamp
    ...
    protected boolean readHead(DataInputStream in, cmppe_pack p)
        throws IOException
    {
        try
        {
    ...
    protected void sendHeader(byte[] buf,cmppe_head ch)
        throws IOException
    {
    ...
    public void cmpp_logout(conn_desc conn)
        throws IOException
    {
    	byte[] buf = new byte[100];
    ...
    public void cmpp_cancel(conn_desc conn, cmppe_cancel cc)
        throws IOException, OutOfBoundsException
    {
    	byte[] buf = new byte[100];
        byte[] _tmp = new byte[512];
    ...
    public void cmpp_active_test(conn_desc conn)
        throws IOException
    {
    	//////////////////////
   ...
   protected void cmpp_send_active_resp(conn_desc conn, int seq)
        throws IOException
    {
        cmppe_head ch = new cmppe_head();
    ...
    public int cmpp_submit(conn_desc conn, cmppe_submit cs)
        throws IOException
    {
        int user_seq = -1;
        ...
        try
        {
        	...
        	System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>");
        	System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>");
        	System.out.println("src_id value is:" + new String(cs.src_terminal_id));
        	System.out.println("socket is:" + conn.sock);
        	System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>");
        	System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>");
        	System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>");
        	System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>");
    ...
    public void readPa(conn_desc con) throws Exception
    {
        ...
        try
        {
            ...
            case -2147483640: 
            		//myLogger.info(FormatSysTime.getCurrentTimeA() + "---------active resp-----------: STAT " + cr.stat);
    ...
    public cmppe_result readResPack(conn_desc conn)
        throws IOException, UnknownPackException, DeliverFailException
    {
        ...
        { 
            ...
            if(pack.pk_head.pk_cmd != 8){
             	...
             	System.out.println("收到网关激活链路。。。。");
            	cmpp_send_active_resp(conn, pack.pk_head.pk_seq); //if pk_cmd =8 then send active_resp
        ...
        case -2147483640: 
            ...
            int success_id = in.readByte();//change at 2008-11-11 in.readByte() -> in.readInt();
            cr.stat = success_id;
     ...
     public void cmpp_send_deliver_resp(conn_desc conn,cmppe_deliver_result cd)
        throws IOException
      {
        cmppe_head ch = new cmppe_head();
        try
    ...
    protected void sendHeader(cmppe_head ch, DataOutput out)
        throws IOException
    {
        try
        {
