...
public  class CMPPSend_sd implements CMPPSend
{
	...
	*/
    public void sendBinaryPicSMS()
    {
    	...
    	int picID = Integer.parseInt(mt.sendContent);
        switch(servermeida)
    	{
    		case 11:
    		case 12:
    ...
    private void sendBinarySMS(int _ringid)
    {
        ...
        for(int i=0;i<buffer.length;i++)
        {
         ...
         if(flag == 0)
         {
             mt.data_coding = 15;
             mt.tp_udhi =0;
             mt.sendContent =  "对不起,没找该编号的图片铃声.(本条免费)";
    ...
    public void send(byte[] submitContent) 
    {
        ...
        try
        {
        	...
        	System.arraycopy(feecode,0,service_code,0,feecode.length);
  		 	byte fee_usertype = 3; //3代表该字段无效 那么fee_terminal_id一定要有
        	...
        	byte[] icpID = SMSIsmgInfo.gd_icpID.getBytes();
        	...
        	byte[] feetype = new byte[2];
      		...
      		byte[] fee = new byte[6];
       		...
       		{
       			mt.spCode = SMSIsmgInfo.gd_ismg_spCode;
       		}
      		...
      		if(dest_cpn.length()<13)
        	{
      		...
      		System.arraycopy(destcpn,0,dest_terminal_id[0],0,destcpn.length);
       		...
       		cmppe_submit sub = new cmppe_submit();//提交短信对
			...
			myLogger.info(FormatSysTime.getCurrentTimeA() + " send msg--spcode:" + mt.spCode + " cpn:" + mt.feeCpn + " linkId:" +  mt.linkID + " fee:" + new String(fee) + " service_code:" + new String(service_code) +  " content:" +  new String(msg_content));
