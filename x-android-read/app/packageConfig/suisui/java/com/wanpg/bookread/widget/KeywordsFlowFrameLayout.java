 ...
 * @author wanpg
 */
public class KeywordsFlowFrameLayout extends FrameLayout implements OnGlobalLayoutListener {
    ...
    private int[] colors = new int[]{
            R.color.shelf_back_00,
            R.color.shelf_back_01,
            R.color.shelf_back_02,
            R.color.shelf_back_03,
            R.color.shelf_back_04,
            R.color.shelf_back_05,
    ...
    public KeywordsFlowFrameLayout(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        this.context = context;
        init();
    }

    public KeywordsFlowFrameLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.context = context;
        init();
    }

    public KeywordsFlowFrameLayout(Context context) {
        super(context);
        this.context = context;
        init();
    }
...
//             + " yItem=" + yItem + "---------------------------");  
            ...
            for (int i = 0; i < size; i++) {
                ...
                // 随机颜色  
                int colorId = (int) (Math.random() * 10);
                int ranColor = getResources().getColor(colors[colorId]);
                //int ranColor = 0xff000000 | random.nextInt(0x0077ffff);  
                // 随机位置，糙值  
                int xy[] = randomXY(random, listX, listY, xItem);
                // 随机字体大小  
                //int txtSize = DisplayUtil.dp2px(context, 15);//= TEXT_SIZE_MIN + random.nextInt(TEXT_SIZE_MAX - TEXT_SIZE_MIN + 1);  
                // 实例化TextView  
                final TextView txt = new TextView(context);
                txt.setOnClickListener(itemClickListener);
                txt.setText(keyword);
                txt.setPadding(DisplayUtil.dp2px(16), DisplayUtil.dp2px(5), DisplayUtil.dp2px(16), DisplayUtil.dp2px(5));
                txt.setTextColor(getResources().getColor(R.color.white));
                txt.setBackgroundColor(ranColor);
                txt.setTextSize(15);
                //txt.setShadowLayer(1, 1, 1, 0xdd696969);  
                txt.setGravity(Gravity.CENTER);
                txt.setOnTouchListener(new MyOnTouchListener(ranColor));

//                txt.setBackgroundColor(Color.RED);
                // 获取文本长度  
                Paint paint = txt.getPaint();
                int strWidth = (int) Math.ceil(paint.measureText(keyword)) + 2 * DisplayUtil.dp2px(16);
    ...
    private class MyOnTouchListener implements OnTouchListener {
        ...
        public MyOnTouchListener(int color) {
        ...
        public boolean onTouch(View v, MotionEvent event) {
            ...
            if (event.getAction() == MotionEvent.ACTION_DOWN) {
                v.setBackgroundColor(bgColor >> 2);
