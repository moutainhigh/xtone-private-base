...
public class StoreCategoryListFragment extends BaseFragment {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		scname = bundle.getString("scname");
		scid = bundle.getInt("scid");
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if(!resetView(v_main)){
			...
			((TextView) v_main.findViewById(R.id.tv_title)).setText(scname);
	...
	private void initData() {
		...
		new BackTask() {
			...
			public String doInThread() {
				// TODO Auto-generated method stub
				if (listBooks == null) {
					listBooks = ShuPengApi.getSecondCategoryBookList(pIndex, pSize, cid, scid);
				}
				if (listBooks == null) {
					try {
						Thread.sleep(1500);
			...
			public void onRefresh() {
				...
				new BackTask() {
					...
					public String doInThread() {
						// TODO Auto-generated method stub
						pIndex = 1;
						listTmp = ShuPengApi.getSecondCategoryBookList(pIndex, pSize, cid, scid);
						if (listTmp == null) {
							try {
								Thread.sleep(2000);
							} catch (InterruptedException e) {
