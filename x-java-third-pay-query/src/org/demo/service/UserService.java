...
public class UserService {
		...
		if (tmpId > 0) {
			...
			try {
				...
				ps = con.prepareStatement(
						"insert into tbl_thirdpay_cp_users (id,username,pwd,email,add_time) values (?,?,md5(?),?,?)");
				int m = 1;
				ps.setLong(m++, tmpId);
	...
	// try {
	// con = ConnectionService.getInstance().getConnectionForLocal();
	// ps = con.prepareStatement("select id from tbl_thirdpay_cp_users where
	...
	public User checkLoginUser(User user) { // 通过username和password查询User(检查能否登录)
		...
		try {
			...
			ps = con.prepareStatement(
					"select * from tbl_thirdpay_cp_users where username=? and pwd=md5(?) and isAvail=1");
			int m = 1;
			ps.setString(m++, user.getUserName());
			ps.setString(m++, user.getPassword());
			rs = ps.executeQuery();
	...
	public static List<Daily> getDailyByAppkeys(User user) {
		...
		/*	sql = "select FROM_UNIXTIME(id/1000/1000000, '%Y-%m-%d') AS date,sum(price) as price,GROUP_CONCAT(DISTINCT appkey) AS appkey from log_success_pays where appkey='"
			...
			/*sql = "SELECT FROM_UNIXTIME(log_success_pays.`id`/1000/1000000, '%Y-%m-%d') AS DATE,SUM(price) AS price,GROUP_CONCAT(DISTINCT appkey) AS appkey,COUNT(DISTINCT payUserIMEI) AS payUserIMEI,COUNT(tbl_imei_users.`id`) AS newUsers FROM log_success_pays,tbl_imei_users WHERE log_success_pays.`appKey`= '"
					+appkeys+"' AND log_success_pays.`id` = tbl_imei_users.`addTime`  GROUP BY DATE ORDER BY DATE DESC";
			*/
			...
			while (rs.next()) {
				...
				daily.setNewUsers(rs.getInt("newUsers"));
				listDaily.add(daily);
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
