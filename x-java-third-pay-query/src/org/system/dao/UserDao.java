...
import org.system.constant.Constant;
import org.system.database.JdbcControl;
import org.system.database.QueryCallBack;
...
public class UserDao
{
	...
	public Map<String, Object> loadUser(int pageIndex,int groupId,String userName,String nickName)
	{
		...
		map.put("list", new JdbcControl().query((groupId>0 ? sql :sql2).replace(Constant.CONSTANT_REPLACE_STRING, (groupId>0 ? " a.group_id,b.*,b.`nick_name` group_name " : " a.*,b.`nick_name` group_name ")) + query + order + limit, new QueryCallBack()
		{
			@Override
			public Object onCallBack(ResultSet rs) throws SQLException
			{
	...
	{
		String sql = "select " + Constant.CONSTANT_REPLACE_STRING + " FROM  daily_config.`tbl_group_user` a LEFT JOIN  daily_base.tbl_user b ON a.`user_id` = b.`id` where 1=1 ";
