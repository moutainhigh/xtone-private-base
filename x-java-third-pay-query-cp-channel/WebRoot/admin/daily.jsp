...
// 					System.out.println(appkey.replace(",", "' or appkey='"));
					...
					sql = "select FROM_UNIXTIME(id/1000/1000000, '%Y-%m-%d) AS date,sum(price) as price,GROUP_CONCAT(DISTINCT appkey) AS appkey from log_success_pays where appkey='"+appkey+"' and releaseChannel='"+user.getReleaseChannel()+"' group by date ORDER BY date DESC";
					ps = con.prepareStatement(sql);
					rs = ps.executeQuery();
					while (rs.next()) {
						daily = new Daily();
