...
public class UserService {
  ...
  public User checkLoginUser(User user) { // 通过username和password查询User(检查能否登录)
    ...
    try {
      ...
      ps = con
          .prepareStatement("select * from tbl_thirdpay_cp_channel_users where username=? and pwd=md5(?) and isAvail=1");
      int m = 1;
      ps.setString(m++, user.getUserName());
  ...
  public static List<Pays> selectByAppkeyAndReleaseChannel(Apps apps, String releaseChannel) { // 通过Apps表中的appkey查询所有的Pays(用于前台显示)
    ...
    try {
      ...
      ps = con
          .prepareStatement("select FROM_UNIXTIME(id/1000/1000000, '%Y-%m-%d %h:%i:%s') AS id,price,paychannel,ip,'' as payInfo,releasechannel,"
