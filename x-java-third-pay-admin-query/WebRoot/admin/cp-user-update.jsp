...
<script language="javascript">
	...
	function updateAjax() {
		...
		$.ajax({
			...
			url : "cp-user-update-commit.jsp",
			async : true,
			data : JSON.stringify(date),
			dataType : "json",
			success : function(msg) {
				alert(action+'成功!');
				location.href = 'cp-users.jsp';
			},
			error : function(msg) {
				alert(action+'失败!');
...
<body>
	<%
		...
		try {
			...
			String sql = "SELECT username,pwd,email,isAvail,isAdmin,lastLogin,ADDTIME,cpId FROM `tbl_thirdpay_cp_users` WHERE id=?";
	...
	<div style="text-align: center; margin-top: 200px">
		...
		<form id="form1" name="form1">
			...
			<span>CPID： </span><input type="text"
				...
				class="btn1" type="button"
				value="取消修改" onclick="window.location.href='cp-users.jsp'">
