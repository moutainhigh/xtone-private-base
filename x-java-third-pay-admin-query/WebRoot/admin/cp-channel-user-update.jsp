...
<script language="javascript">
	...
	function updateAjax() {
		...
		var date = {
				...
				cpId : $("#cpId").val(),
				relchannel : $("#relchannel").val(),
				rbamoumt : $("#rbamoumt").val()==""?0 : $("#rbamoumt").val(),
				redscale : $("#redscale").val()==""?0 : $("#redscale").val()
			};
		$.ajax({
			type : "post",
			contentType: "application/json; charset=utf-8", 
			url : "cp-channel-user-update-commit.jsp",
			async : true,
			data : JSON.stringify(date),
			dataType : "json",
			success : function(msg) {
				alert(action+'成功!');
				location.href = 'cp-channel-users.jsp';
			},
			error : function(msg) {
				alert(action+'失败!');
...
<body>
	<%
		...
		try {
			...
			String sql = "SELECT username,pwd,email,isAvail,isAdmin,lastLogin,ADDTIME,cpId,releaseChannel,reduceBeginAmoumt,reduceScale FROM `tbl_thirdpay_cp_channel_users` WHERE id=?";
	...
	<div style="text-align: center; margin-top: 200px">
		...
		<form id="form1" name="form1">
			...
			<span>发布渠道： </span><input type="text"
				name="relchannel" id="relchannel" value="<%=rs.getString("releaseChannel") %>" class="input1" readonly="readonly"><br></br>
			<span>扣量起始金额： </span><input type="text"
				name="rbamoumt" id="rbamoumt" value="<%=rs.getInt("reduceBeginAmoumt") %>" class="input1"><br></br>
			<span>扣量比例： </span><input type="text"
				name="redscale" id="redscale" value="<%=rs.getInt("reduceScale") %>" class="input1"><br></br>
				<input
				class="btn1" type="button"
				value="确认修改" onclick="commin()"> <input
				class="btn1" type="button"
				value="取消修改" onclick="window.location.href='cp-channel-users.jsp'">
