...
<%
	...
	try {
		...
		if(Aduser.getType()==2){
			sql = "SELECT pwd FROM `tbl_thirdpay_query_users` WHERE id=?";
			ps = con.prepareStatement(sql);
			ps.setLong(1, Aduser.getId());
			rs = ps.executeQuery();
			if(rs.next()){
				if(Aduser.getPwd().equals(rs.getString("pwd"))){
					sql = "UPDATE `tbl_thirdpay_query_users` SET username=?,pwd=?,email=?,isAdmin=?,isAvail=? WHERE id=?";
				}else{
					sql = "UPDATE `tbl_thirdpay_query_users` SET username=?,pwd=md5(?),email=?,isAdmin=?,isAvail=? WHERE id=?";
				}
			}
			...
			sql = "INSERT INTO `tbl_thirdpay_query_users`(username,pwd,email,isAdmin,isAvail,lastLogin,addTime,cpId) VALUES(?,md5(?),?,?,?,?,?,?)";
			ps = con.prepareStatement(sql);
