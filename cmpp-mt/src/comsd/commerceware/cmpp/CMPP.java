...
public final class CMPP
{
	...
	//连接网关
	public void cmppConnectToIsmg(String host, int port, ConnDesc conn)
        throws IOException
    {
        Socket s = null;
        try
  ...
  //login ismg
   public void cmppLogin(ConnDesc conn, CmppeLogin cl)
        throws IOException, OutOfBoundsException
    {
      ...
      //加密
        ...
        try
        {
            ...
            byte[] bufMd5 = md5.getMD5ofStr(md5Byte, lengthMd5);// get md5 for icpAuth
            bodyLen +=tools.strcpy(buf,bufMd5,bodyLen+ch.pkLen); // add icpAuth
            
            bodyLen +=tools.strcpy(buf,cl.icpVersion,bodyLen+ch.pkLen); //add icpVersion
            bodyLen +=tools.strcpy(buf,cl.icpTimestamp,bodyLen+ch.pkLen); // add icpTimestamp
             // bodyLen +=tools.strcpy(buf,(short)cl.icpTimestamp,bodyLen+ch.pkLen); // add icpTimestamp
    ...
    protected boolean readCountByte(DataInputStream in, byte buf[], int len)
        throws IOException
    {
        try
        {
    ...
    protected boolean readHead(DataInputStream in, CmppePack p)
        throws IOException
    {
        try
        {
    ...
    protected boolean sendUnknowncountByte(DataOutputStream out, byte buf[])
        throws IOException
    {
        int i = 0;
        try
    ...
    protected void sendHeader(byte[] buf,CmppeHead ch)
        throws IOException
    {
    ...
    public void cmppLogout(ConnDesc conn)
        throws IOException
    {
    	byte[] buf = new byte[100];
    ...
    public void cmppCancel(ConnDesc conn, CmppeCancel cc)
        throws IOException, OutOfBoundsException
    {
    	byte[] buf = new byte[100];
        byte[] Tmp = new byte[512];
    ...
    public void cmppActiveTest(ConnDesc conn)
        throws IOException
    {
   ...
   protected void cmppSendActiveResp(ConnDesc conn, int seq)
        throws IOException
    {
        CmppeHead ch = new CmppeHead();
    ...
    public int cmppSubmit(ConnDesc conn, CmppeSubmit cs)
        throws IOException
    {
        int userSeq = -1;
    ...
    public CmppeResult readResPack(ConnDesc conn)
        throws IOException, UnknownPackException, DeliverFailException
    {
        ...
        { 
            ...
            if(pack.pkHead.pkCmd != 8)
                break;
           // cmppSendActiveResp(conn, pack.pkHead.pkSeq); //if pkCmd =8 then send activeResp
        } while(true);
        ...
        case -2147483640: 
            ...
            int successId = in.readByte();//change at 2008-11-11 in.readByte() -> in.readInt();
            cr.stat = successId;
     ...
     public void cmppSendDeliverResp(ConnDesc conn,CmppeDeliverResult cd)
        throws IOException
      {
        CmppeHead ch = new CmppeHead();
        try
    ...
    protected void sendHeader(CmppeHead ch, DataOutput out)
        throws IOException
    {
        try
        {
