...
public class CheckPayInfo {
	...
	public static void UpdataInfo(String ownOrderId) {
		...
		try {
			...
			if (!ps.execute()) {
				LOG.info(ownOrderId + " 数据状态更新成功为1");
			}
	...
	public static void InsertInfo(String ownOrderId, String notify_url) {
		// ThreadPool.mThreadPool.execute(new ForwardLogBean(1002, ownOrderId,
		// notify_url, "success", "result_status",
		// "send_time", "rsp_time", "send_url", "send_header", "body",
		// "rsp_status","rsp_header", "rsp_body", "task_id",
		// "key_type", "id_type"));
	...
	public static String Check_next_forwardTime(){
